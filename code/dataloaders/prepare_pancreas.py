...
def parse_args():
    ...
    parser.add_argument('--base_dir', type=str, default='./Pancreas-CT-selected', help='base dir for data')
...
def main():
    ...
    for file in tqdm.tqdm(train_list):
        ...
        filename = "image"+file.split('_')[-1]
        sitk.WriteImage(fg_image, os.path.join(output_dir, 'images', filename + '.nii.gz'))
        fg_label.SetDirection(label_t.GetDirection())
        fg_label.SetSpacing(label_t.GetSpacing())
        sitk.WriteImage(fg_label, os.path.join(output_dir, 'labels', filename + '.nii.gz'))
