...
class Decoder(nn.Module):
    ...
    def forward(self, features):
        ...
        fg_feats, bg_feats = self.cm(out_seg)
...
class ContraModule(nn.Module):
    def __init__(self, channel, is_enc=False):
        super(ContraModule, self).__init__()

        self.activation_head = nn.Conv3d(channel, 1, 3, padding=1, stride=2, bias=False)
        self.bn_head = nn.BatchNorm3d(1)
    ...
    def forward(self, x):
        ...
        bg_feats = (1 - ccam_) / (H * W * Z)  # [N, 1, H*W*Z]
