    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_seg_dice = dice_loss(
                output_soft[:labeled_bs, 1, ...], label_batch[:labeled_bs, ...] == 1
            )
            ...
            optimizer.step()
            dc = metrics.dice(
                torch.argmax(output_soft[:labeled_bs], dim=1), label_batch[:labeled_bs]
            )
            logging.info(
                "iteration %d : loss : %03f, loss_d: %03f, loss_contra: %03f, dice: %03f"
                % (iter_num, loss, loss_seg_dice, loss_contra, dc)
            )
            ...
            if iter_num >= 800 and iter_num % 200 == 0:
                ...
                image = output_soft[0, 0:1, :, :, 20:61:10].permute(3, 0, 1, 2).repeat(1, 3, 1, 1)
                grid_image = make_grid(image, 5, normalize=False)
                writer.add_image("train/Predicted_label", grid_image, iter_num)

                image = (
                    label_batch[0, :, :, 20:61:10]
                    .unsqueeze(0)
                    .permute(3, 0, 1, 2)
                    .repeat(1, 3, 1, 1)
                )
