...
parser.add_argument("--exp", type=str, default="fbapnet", help="exp_name")
...
snapshot_path = args.root_path + "model/{}_{}_{}_{}_labeled/{}".format(
    args.dataset_name, args.exp, args.dim, args.labelnum, args.model
)
...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            model.train()
            fg_feats, bg_feats, outputs = model(volume_batch)
            ...
            for idx in range(num_outputs):
                ...
                loss1 += max_loss(fg_feats[idx])
                loss2 += max_loss(bg_feats[idx])
                neg_loss += min_loss(fg_feats[idx], bg_feats[idx])
            ...
            loss = args.lamda * loss_seg_dice + consistency_weight * loss_consist + loss_contra
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            logging.info(
                "iteration %d : loss : %03f, loss_d: %03f, loss_cosist: %03f, loss_contra: %03f"
                % (iter_num, loss, loss_seg_dice, loss_consist, loss_contra)
            )
            ...
            if iter_num >= 800 and iter_num % 200 == 0:
                ...
                snapshot_img = torch.zeros(
                    size=(
                        D,
                        3,
                        (num_outputs + 2) * H + (num_outputs + 2) * ins_width,
                        W + ins_width,
                    ),
                    dtype=torch.float32,
                )
                ...
                for idx in range(num_outputs + 2):
                    ...
                    snapshot_img[
                        :,
                        :,
                        begin_grid * H + ins_width : begin_grid * H + begin_grid * ins_width,
                        :,
                    ] = 1
                ...
                for idx in range(num_outputs):
                    ...
                    snapshot_img[
                        :, 0, begin * H + begin * ins_width : end * H + begin * ins_width, :W
                    ] = y_ori[idx][labeled_bs:][0, 1].permute(2, 0, 1)
                    snapshot_img[
                        :, 1, begin * H + begin * ins_width : end * H + begin * ins_width, :W
                    ] = y_ori[idx][labeled_bs:][0, 1].permute(2, 0, 1)
                    snapshot_img[
                        :, 2, begin * H + begin * ins_width : end * H + begin * ins_width, :W
                    ] = y_ori[idx][labeled_bs:][0, 1].permute(2, 0, 1)
            ...
            if iter_num >= 800 and iter_num % 200 == 0:
                model.eval()
                if args.dataset_name == "LA":
                    dice_sample = test_patch.var_all_case(
                        model,
                ...
                if dice_sample > best_dice:
                    ...
                    save_mode_path = os.path.join(
                        snapshot_path, "iter_{}_dice_{}.pth".format(iter_num, best_dice)
                    )
