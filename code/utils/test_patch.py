...
def var_all_case(
    model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"
):
    if dataset_name == "LA":
        with open("./data/LA/test.list", "r") as f:
            image_list = f.readlines()
        image_list = [
            "./data/LA/2018LA_Seg_Training Set/" + item.replace("\n", "") + "/mri_norm2.h5"
        ...
        with open("./data/Pancreas/test.list", "r") as f:
            image_list = f.readlines()
        image_list = [
            "./data/Pancreas/Pancreas_h5/" + item.replace("\n", "") + "_norm.h5"
            for item in image_list
        ]
...
def test_all_case(
    ...
    for image_path in loader:
        ...
        if num_outputs > 1:
            prediction_average, score_map_average = test_single_case_average_output(
                model, image, stride_xy, stride_z, patch_size, num_classes=num_classes
            )
        if nms:
        ...
        if metric_detail:
            ...
            if num_outputs > 1:
                print(
                    ...
                    % (
                        ith,
                        single_metric_average[0],
                        single_metric_average[1],
                        single_metric_average[2],
        ...
        if save_result:
            ...
            nib.save(
                nib.Nifti1Image(score_map[0].astype(np.float32), np.eye(4)),
                test_save_path + "%02d_scores.nii.gz" % ith,
            )
            if num_outputs > 1:
                nib.save(
                    nib.Nifti1Image(prediction_average.astype(np.float32), np.eye(4)),
                    test_save_path + "%02d_pred_average.nii.gz" % ith,
                )
                nib.save(
                    nib.Nifti1Image(score_map_average[0].astype(np.float32), np.eye(4)),
